...
if __name__ == "__main__":
    ...
    if conf.oneD: # set 1D curvature parameters for QED reactions
        ...
        mc.r_gap  = conf.rad_pcap # cutoff distance for pair creation
    ...
    for lap in range(lap, conf.Nt + 1):
        ...
        #        sch.operate( dict(name='mpi_cur_flt',  solver='mpi',   method='j',                                    nhood='all',   ) ) # update virtual tiles
        #        sch.operate( dict(name='upd_bc',       solver='tile',  method='update_boundaries',args=[grid, [0,] ], nhood='local', ) ) # local tiles have up to date internal grid values for j
        #        MPI.COMM_WORLD.barrier()
        #    sch.operate( dict(name='filter',   solver='flt',   method='solve',    nhood='local', ) )
        ...
        if lap % conf.plot_interval == 0 and live_plot:
            ...
            if conf.qed_mode and sch.is_master:
                ...
                #--------------------------------------------------
                if True: # TODO synchrotron emission peak
                    g = conf.gam_rad
                ...
                if False: # TODO debug; check energy losses
                    ...
                    print('Psyn:', Psyn, ' Esyn:', Esyn, 'emeas/esyn', toolset.storage.data['ene_ph'][-1]/Esyn)
