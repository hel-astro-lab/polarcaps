...
if __name__ == "__main__":
    ...
    axs[0,0].set_ylabel(r"$\chi_{\pm, min}$")
    ...
    def norm_fun(x):
        ...
        return bkn_pow_smooth(x, 0.01876, 0.103, 0.931, 0.0475, delta=1.08)
    ...
    if True:
        ...
        def model_broken_plaw(theta, x):
            def bkn_plaw_smooth(x, A, x_b, a_1, a_2, delta=1):
                a_1 *= -1
                a_2 *= -1
                return A*(x/x_b)**(-a_1) * (0.5*(1+(x/x_b)**(1/delta)))**((a_1-a_2)*delta)

            A, x_b, a_1, a_2, delta = theta

            a_1 = 0.931
            a_2 = 0.047

            return bkn_plaw_smooth(x, A, x_b, a_1, a_2, delta=delta)
        ...
        def log_prior(theta):
            ...
            if A < 0.0 or A > 1.0: return -np.inf
            if x_b < 0.01 or x_b > 3.0: return -np.inf
            if a_1 < 0.0 or a_1 > 3.0: return -np.inf
            if a_2 < 0.0 or a_2 > 3.0: return -np.inf
            if delta < 0.0 or delta > 3.0: return -np.inf
        ...
        fit_params = {
                'model':   model_broken_plaw,
                'names':   ['A', 'x_b', 'a_1', 'a_2', 'delta'],
                'initial': [0.015, 0.1,  1.0, 0.03, 1.45],
                }
        ...
        axs[0,0].plot(chi, fit_params['model'](soln.x, chi), lw=1.5, color='C5', linestyle='dashed')

        plot_error( model_broken_plaw(soln.x, chi),
