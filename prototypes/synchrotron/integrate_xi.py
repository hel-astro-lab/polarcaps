...
if __name__ == "__main__":
    ...
    axs[0,0].set_ylabel(r"$\chi_x$")
    axs[1,0].set_ylabel(r"$\chi_x$")

    axs[0,1].set_xlabel(r"$\chi_x$")
    axs[1,1].set_xlabel(r"$\chi_x$")

    axs[0,1].set_ylabel(r"$F |_{\chi_\pm = \mathrm{const}}$")
    axs[1,1].set_ylabel(r"$F |_{\chi_\pm = \mathrm{const}}$")

    axs[0,2].set_xlabel(r"$\chi_\pm$")
    axs[1,2].set_xlabel(r"$\chi_\pm$")
    axs[0,2].set_ylabel(r"$F |_{\chi_x = \mathrm{const}}$")
    ...
    if True:
        ...
        if 'xi' in f:
            ...
            #chiph = np.linspace(1,chiph_dim+1,chiph_dim+1) # TODO commented out
            chiph = np.logspace(np.log10(chipa_min),np.log10(chipa_max),size_particle_chi) # TODO new addition
    ...
    def plot_xi(row, xi):
        ...
        axs[row,0].contourf(chipa, chiph, xi[:,:],
                          cmap=get_cmap('plasma'),
                          levels=[1e-3, 3e-3, 1e-2, 3e-2, 1e-1, 0.3, 0.6, 0.8, 0.9, 1],
                          norm=LogNorm(),
                          vmin=1e-3,
                          vmax=1.0,
                          )
    ...
    if False:
        ...
        X = np.zeros(( (size_particle_chi//4)**2, 2))
        y = np.zeros(( (size_particle_chi//4)**2))

        for i in range(0, size_particle_chi, 4):
            for j in range(0, size_particle_chi, 4):
                X[i+j,0] = chipa[i]
                X[i+j,1] = chiph[j]
                y[i+j] = xi[j,i]
    ...
    for i in range(0, size_particle_chi):
        for j in range(0, size_particle_chi):
            xi_sr[j, i] = estimate_sr( chipa[i], chiph[j] )
